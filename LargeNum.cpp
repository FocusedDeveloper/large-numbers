...
const LargeNum& LargeNum::operator=(const LargeNum& orgNum)
{
    ...
    if(orgNum.num)                          // copy the data from int array directly into this object
    {
        sign = orgNum.sign;
        arraySize = orgNum.arraySize;
        num = new int[arraySize];
...
//            delete[] answer.num;
    ...
    if(myOp == add )
    {
        if(numNegatives != 1)       // the number of negatives is divisible by 2, add the numbers
        {
            ...
            addition(intArray1, size1 , intArray2, size2, intArray3, size3);
        }else{                          // If the number of negatives is not divisible by 2, subtract the numbers
            subtraction(intArray1, size1, sign1, intArray2, size2, intArray3, size3);
        }
    }
    ...
    {
        multiplication(intArray1, size1 , intArray2, size2, intArray3, size3);
    }else if(myOp == divide )
    {
        division(num,arraySize,sign,num2.num,num2.arraySize,intArray3,size3);
    }else if(myOp == mod )
    {
        //answer = division(num,arraySize,sign,num2.num,num2.arraySize,intArray3,size3);
    }
//    cout<<"Size of answer is: "<<answer.arraySize<<endl;
    LargeNum answer(intArray3, size3, sign3);
    answer.reSizeArray();   // resize the answer array to get rid of any wasted space
//    cout<<"Size of answer is POST: "<<answer.arraySize<<endl;
    cout<<endl<<"About to print answer"<<endl;
    answer.print();
    cout<<endl;
    ...
    try
    {
        for(int i = 0; i < size1; i++)               // For the entire array...
        {
            ...
            }else
            {
                ...
                if(x < 0 || x >= 10)                // this statement should always evaluate to false, could use assertion...
                    throw(invalidValue("num1",i,num1[i]));
            }
            ...
            }else
            {
                ...
                if(y < 0 || y >= 10)  {                          // this statement should always evaluate to false, could use assertion...
                    throw(invalidValue("num2",i,num2[i]));
                }
            }
...
void LargeNum::multiplication(const int num1[], const int size1,const int num2[], const int size2, int product[], const int size3) const
{
    ...
    try{
        ...
        for(int i = 0; i < size2; i++)            // for each number in the 2nd array...
        {
            ...
            {
                addition(tempSum, size3 , num1, size1, tempSum, size3); // add num1 to temp sum factor amount of times
            }
...
LargeNum LargeNum::division(const int num1[], const int size1, Sign sign1,int num2[], const int size2, int quotient[], const int size3) const
...
// right now x/1 will take x number of calculations...
{
    ...
    {
        // count how many times temp2 is subtracted from temp1
        divCounter++;
        ...
        if(divCounter == 9 ){                    // if the counter reaches 9...
                                                                            // was doing some last minute testing and was getting the wrong answer when divisible by 9
//            cout<<"About to tally div"<<endl;
//            cout<<"divCouter: "<<divCounter<<endl;
            addition(quotient,size3, divCounterPtr, 1, quotient, size3);    // add the counter to the current quotient
            divCounter = 0;                                                     // and reset the counter
        }
    }
    //cout<<"Pre addition Final"<<endl;
//    cout<<"divCounter: "<<divCounter<<endl;
    addition(quotient,size3, divCounterPtr, 1, quotient, size3);            // add the counter to the quotient
    delete[] diff;
...
// POST: MSG has been dropped from num
void LargeNum::dropMostSigDig(int num[], int size1) const
{
    ...
    try
    {
        while(!mostSigDig && i >= 0)           // until the MSG is found, or out of index...
        {
        mostSigDig = mostSigDig + num[i];       // find the MSG...
        if(mostSigDig)                          // once found...
        {
            num[i]= 0;                          // set MSG equal to zero
        }
        i--;
        }
    }
